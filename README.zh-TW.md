...
1. 如果還沒裝，先安裝這套 kit
2. 幫目標 repo 自動建立 context
3. 幫你輸出一段開始前要對 Codex 說的話

如果你要初始化的是「目前所在資料夾」，可以直接：

```bash
./quickstart.sh
```

如果你要同時套用建議的全域 policy（取代預設 AGENTS），加上 `--adopt-policy`：

```bash
./quickstart.sh --repo /path/to/your/repo --adopt-policy
```

如果 kit 已經裝好，只要初始化，直接加 `--skip-install`：
...
| 指令/旗標 | 代表什麼 |
| --- | --- |
| `init-repo` 不加 `--repo` | 會初始化「現在所在目錄」。 |
| `init-repo --repo /path/to/repo` | 會初始化指定目錄（你在別的目錄時很實用）。 |
| `install.sh` | 安全安裝：保留你既有的 `~/.codex/AGENTS.md`。 |
| `install.sh --adopt-policy` | 安裝並把建議 policy 寫入 `~/.codex/AGENTS.md` 成為預設。 |

...
它讓 Codex 不再只是一次性提示，而是用分階段的方式完成工作，並且多了一個可在實作前先選路徑的 `factory-router`、用來縮小 blast radius 的 safety layer，以及可把可重複使用經驗沉澱下來的 learning layer。
...
11. 撰寫 retro
12. 把可重用的 learnings 提升成持久 guidance

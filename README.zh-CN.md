...
1. 若尚未安装，则先运行安装
2. 自动初始化目标 repo 的 context
3. 输出建议你先对 Codex 说的那一句

如果目标 repo 就是当前目录，可省略 `--repo`：

```bash
./quickstart.sh
```

如果你希望直接启用建议的全局策略（替换默认 AGENTS），加 `--adopt-policy`：

```bash
./quickstart.sh --repo /path/to/your/repo --adopt-policy
```

如果 kit 已经装好，只需要初始化，使用 `--skip-install`：
...
| 指令/旗标 | 含义 |
| --- | --- |
| `init-repo` 不加 `--repo` | 在「当前目录」初始化。 |
| `init-repo --repo /path/to/repo` | 在指定目录初始化（从其他目录启动时很有用）。 |
| `install.sh` | 安全模式安装：保留你现有的 `~/.codex/AGENTS.md`。 |
| `install.sh --adopt-policy` | 安装并立刻把建议 policy 写入 `~/.codex/AGENTS.md` 并设为默认。 |

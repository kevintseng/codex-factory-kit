...
对于非 trivial 的工作，你也可以先从 `factory-router` 开始，判断任务应该保持 lightweight mode，还是进入 full loop。
...
第一个 vNext wedge 以 `factory-router` 的形式 shipped。

当你希望在深入执行前先得到一个 rule-based routing decision，可以使用它。它会判断任务应该使用 lightweight mode 还是 full loop，以及接下来是否需要 `office-hours-codex`、`sprint-conductor`、`review-gate`、`qa-runtime`、`document-release`、`retro`。

`factory-router` 是 soft orchestration，不宣称平台级强制执行。
...
5. 如果你想先得到 routing decision，可以先用 `factory-router`。

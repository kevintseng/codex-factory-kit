...
Codex Factory Kit は、Codex community 向けのオープンソース workflow kit です。

これは、実際の repo で Codex を使い、単発のプロンプト集よりも明確な運用モデルを求める人のためのものです。

Languages: [English](README.md) | [繁體中文](README.zh-TW.md) | [简体中文](README.zh-CN.md) | [日本語](README.ja.md) | [한국어](README.ko.md)

既存の Codex capabilities を、staged execution、repo-local working memory、structured review、runtime QA evidence、そして小さなタスク向けの lightweight mode によって、より意図的に使えるようにします。
...
non-trivial な作業では、`factory-router` から始めて、そのタスクが lightweight mode に留まるべきか、full loop に入るべきかを先に判断することもできます。
...
最初の vNext wedge は `factory-router` として shipped されます。

深い作業に入る前に rule-based な routing decision が欲しいときに使います。タスクが lightweight mode か full loop かを判定し、次に `office-hours-codex`、`sprint-conductor`、`review-gate`、`qa-runtime`、`document-release`、`retro` のどれが必要かを示します。

`factory-router` は soft orchestration であり、platform-level の強制実行を主張しません。
...
このプロジェクトは、Codex の platform primitives を置き換えるものではなく、実際の仕事で Codex をよりうまく使うための実用的なコミュニティ貢献として作られています。
...
5. 先に routing decision が欲しい場合は `factory-router` を使います。
...
各 skill ディレクトリには、インストール後の Codex surface で使う `agents/openai.yaml` のインターフェース定義も含まれます。

...
## 초보자용 바로 시작하기（복붙만 하면 됨）

개발 경험이 적어도 먼저 이 2줄만 실행하세요.
...
1. 설치되지 않았으면 kit을 설치
2. 대상 repo context 초기화
3. Codex에 먼저 말할 한 줄 텍스트 출력

대상 repo에 이미 들어가 있는 상태라면 `--repo` 없이 실행합니다.
...
권장 policy를 즉시 적용하려면 `--adopt-policy`를 추가하세요.

```bash
./quickstart.sh --repo /path/to/your/repo --adopt-policy
```

이미 kit이 설치된 환경이라면 bootstrap만 할 때 `--skip-install`을 사용하세요.
...
- `--repo PATH`: 대상 repo를 직접 지정
- `--repo` 미지정: 현재 실행 위치를 대상 repo로 사용

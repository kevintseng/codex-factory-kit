...
## 초보자용 바로 시작하기 (수동 설정 최소화)

개발 경험이 많지 않아도 아래 명령만 알면 됩니다.
...
1. 필요하면 `install.sh`를 실행해 kit을 설치
2. 지정한 repo의 context를 자동 초기화
3. Codex 시작 문구를 출력

대상 repo가 현재 폴더라면 `--repo`를 생략할 수 있습니다.

```bash
./quickstart.sh
```

추천 global policy를 바로 기본으로 적용하려면 `--adopt-policy`를 붙입니다.

```bash
./quickstart.sh --repo /path/to/your/repo --adopt-policy
```

kit 설치가 이미 되어 있는 경우엔 초기화만 할 때 `--skip-install`을 사용하세요.
...
| 명령/플래그 | 의미 |
| --- | --- |
| `init-repo` 에서 `--repo` 미지정 | 현재 폴더를 초기화 |
| `init-repo --repo /path/to/repo` | 지정한 경로를 초기화(다른 폴더에서 실행할 때 유용) |
| `install.sh` | 기존 `~/.codex/AGENTS.md`를 건드리지 않는 안전 모드 |
| `install.sh --adopt-policy` | `~/.codex/AGENTS.md`를 kit policy로 즉시 덮어씀 |
